...
{
    /**
     * Menampilkan halaman utama dashboard orang tua perkuliahan V2.
     */
    public function index(Request $request): Response
    {
        ...
        $mahasiswa = Mahasiswa::with(['kelas.prodi', 'kelas.semester', 'pembimbingAkademik'])
    ...
    /**
     * Mengubah mahasiswa aktif yang sedang dipantau oleh orang tua.
     */
    public function switchChild(Request $request): RedirectResponse
    {
        $request->validate([
            'child_id' => 'required|exists:mahasiswas,id',
        ]);

        $parent = auth()->guard('orang_tua')->user();

        // Security check: ensure the child belongs to this parent
        $hasChild = $parent->mahasiswas()->where('mahasiswas.id', $request->child_id)->exists();

        if (! $hasChild) {
            return redirect()->back()->with('error', 'Akses tidak sah untuk mahasiswa ini.');
        }

        session(['user.activeChildId' => $request->child_id]);

        return redirect()->back()->with('success', 'Mahasiswa aktif berhasil diubah.');
    }
}
