...
// Initialize materi and pustaka for 14 meetings
const initialMateri = {}
...
const submit = () => {
  form.post(route('v2.dosen.kontrak.store'), {
    onSuccess: () => {
      // redirects to index
    }
  })
...
<template>
  <AdminLayout>
    <Head title="Isi Kontrak Perkuliahan" />
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-slate-500 text-sm mt-0.5">
            Lengkapi materi dan pustaka perkuliahan untuk semua pertemuan kelas.
          </p>
        </div>
      </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="flex flex-col justify-between md:items-end text-sm text-slate-600 space-y-2 md:space-y-0">
              <div class="md:text-right">
                ...
                <p>Tahun Akademik: <strong class="text-[#4B49AC]">{{ tahun }}</strong></p>
              </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
          ...
          <Card v-for="i in 7" :key="i" class="border-none shadow-sm hover:shadow-md transition-shadow">
            <CardHeader class="p-4 pb-2 bg-slate-50/50 rounded-t-lg">
              <CardTitle class="text-sm font-bold text-slate-700 flex items-center gap-1.5">
                <span class="w-5 h-5 rounded-full bg-slate-200 text-slate-700 flex items-center justify-center text-xs font-mono">
                  {{ i }}
                </span>
                Pertemuan ke-{{ i }}
              </CardTitle>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-slate-200">
          ...
          <div class="flex gap-3">
            <!-- Helper notice to switch tabs -->
            <div v-if="activeTab === '1-7'" class="hidden sm:flex items-center text-xs text-slate-400 gap-1">
              <AlertCircle class="w-3.5 h-3.5 text-indigo-400" />
              Pastikan Anda mengisi tab berikutnya sebelum menyimpan
            </div>
            <Button
              v-if="activeTab === '1-7'"
              type="button"
              class="bg-[#4B49AC] hover:bg-[#3f3e91] text-white"
              @click="activeTab = '8-14'"
            >
            ...
            <Button
              v-else
              type="submit"
              class="bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1.5 shadow-sm"
              :disabled="form.processing"
            >
              <Save class="w-4 h-4" />
              {{ form.processing ? 'Sedang Menyimpan...' : 'Simpan Kontrak' }}
            </Button>
          </div>
        </div>
      </form>
