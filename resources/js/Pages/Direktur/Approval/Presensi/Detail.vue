...
const submitApprove = () => {
  ...
  form.post(route('v2.direktur.rekap-presensi.approve', props.params), {
    preserveScroll: true,
    onSuccess: () => {
...
const isAllApprovedByWadir = props.absens.every(a => a.setuju_wadir === 1)
</script>
    ...
    <div class="space-y-6">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-2">
          <Link 
            :href="route('v2.direktur.rekap-presensi.index')"
            class="p-1 hover:bg-gray-100 rounded-full transition-colors"
          >
            <ChevronLeft class="w-6 h-6 text-gray-500" />
          </Link>
          <div>
            <h1 class="text-2xl font-bold text-[#1F2937]">Detail Persetujuan Presensi (Pimpinan)</h1>
