...
const submitApprove = () => {
  ...
  form.post(route('v2.direktur.rekap-kontrak.approve', props.params), {
    preserveScroll: true,
    onSuccess: () => {
...
const isAllApprovedByWadir = computed(() => {
  return props.kontraks.length > 0 && props.kontraks.every(k => !!k.setuju_wadir)
})
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 no-print">
        <div class="flex items-center gap-2">
          <Link 
            :href="route('v2.direktur.rekap-kontrak.index')"
          ...
          <div>
            ...
            <p class="text-[#6B7280] text-sm">
              Kelola persetujuan kontrak kuliah tingkat institusi (Wadir).
            </p>
          </div>
        </div>
            ...
            <div class="space-y-1">
              ...
              <div class="flex flex-wrap gap-3">
                <div class="flex items-center gap-1.5 text-sm">
                  ...
                  <Badge :variant="kontraks.every(k => !!k.setuju_kaprodi) ? 'success' : 'warning'">
                    {{ kontraks.every(k => !!k.setuju_kaprodi) ? 'Disetujui' : 'Pending' }}
                  </Badge>
                </div>
                <div class="flex items-center gap-1.5 text-sm">
