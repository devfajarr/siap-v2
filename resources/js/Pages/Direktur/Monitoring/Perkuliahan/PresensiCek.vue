  ...
  <AdminLayout>
    ...
    <div class="space-y-6">
      <div class="flex items-center gap-2">
        <Link 
          :href="route('v2.direktur.monitoring.perkuliahan.detail', jadwal.kelas_id)"
          class="p-1 hover:bg-gray-100 rounded-full transition-colors"
        >
          <ChevronLeft class="w-6 h-6 text-gray-500" />
        </Link>
        <div>
          <h1 class="text-2xl font-bold text-[#1F2937] font-nunito">Cek Presensi Pertemuan {{ rentang }}</h1>
      ...
      <Card class="border-none shadow-sm rounded-lg">
        <CardContent class="p-0">
          <div class="rounded-md border overflow-x-auto mx-4 my-4">
            <Table>
              ...
              <TableBody>
                ...
                <TableRow v-if="Object.keys(presensi).length === 0">
                  <TableCell :colspan="pertemuanRange.length + 2" class="h-32 text-center text-gray-500">
      ...
      <!-- Legend -->
      <div class="flex flex-wrap items-center gap-4 text-xs text-gray-500 bg-white p-4 rounded-lg shadow-sm border border-gray-100">
