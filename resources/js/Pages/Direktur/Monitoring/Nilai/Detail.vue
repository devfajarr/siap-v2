  ...
  <AdminLayout>
    ...
    <div class="space-y-6">
      <div class="flex items-center gap-2">
        <Link 
          :href="route('v2.direktur.monitoring.nilai.index')"
        ...
        <div>
          ...
          <p class="text-[#6B7280] text-sm">
            Monitoring nilai mahasiswa per mata kuliah. Prodi: {{ kelas.prodi?.nama_prodi ?? '-' }}
          </p>
        </div>
      </div>
      ...
      <Card class="border-none shadow-sm rounded-lg">
        <CardHeader>
          <CardTitle class="text-lg font-nunito">Daftar Mata Kuliah</CardTitle>
        </CardHeader>
                ...
                <TableRow v-for="jadwal in jadwals" :key="jadwal.id">
                  ...
                  <TableCell class="text-center">
                    <Link
                      ...
                      :href="route('v2.direktur.monitoring.nilai.cek', { matkul_id: jadwal.matkuls_id, kelas_id: kelas.id, jadwal_id: jadwal.id })"
                    >
                      <Button variant="outline" size="sm" class="h-8 gap-1.5 hover:bg-[#4B49AC] hover:text-white transition-all duration-200 shadow-sm text-xs font-semibold">
                        <Eye class="w-4 h-4" />
                        Lihat Nilai
                      </Button>
                    </Link>
                    <div v-else>
                      <Button variant="outline" size="sm" class="h-8 gap-1.5 cursor-not-allowed opacity-50 text-xs font-semibold" disabled>
                        <Eye class="w-4 h-4" />
                        Lihat Nilai
                      </Button>
