...
import { Eye, FolderSearch, CheckCircle2, ChevronLeft, ChevronRight, XCircle, AlertCircle } from 'lucide-vue-next'
...
<template>
    <Head title="Pembayaran Disetujui" />
    ...
    <AdminLayout>
        <template #header>
            <div class="flex items-center gap-2">
                ...
                <span class="text-gray-800 font-semibold">Pembayaran Disetujui</span>
            </div>
        </template>
        ...
        <div class="py-12 space-y-6">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                    <div>
                        <h1 class="text-2xl font-bold text-[#1F2937]">Bukti Pembayaran Terverifikasi (Lunas)</h1>
                        <p class="text-[#6B7280]">Arsip dan riwayat bukti pembayaran KRS mahasiswa yang telah disetujui.</p>
                    </div>
                    <Badge variant="outline" class="bg-emerald-50 text-emerald-700 border-emerald-300 font-medium px-3 py-1.5 self-start md:self-center">
                        <CheckCircle2 class="w-4 h-4 mr-1.5 inline" /> {{ pembayarans.total }} Pembayaran Lunas
                    </Badge>
                </div>
                                    ...
                                    <template v-if="pembayarans.data.length > 0">
                                        <TableRow v-for="(pembayaran, index) in pembayarans.data" :key="pembayaran.id" class="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <TableCell class="text-center">
                                                <Badge variant="secondary" class="bg-emerald-100 text-emerald-800 hover:bg-emerald-100 font-normal px-2.5 py-0.5">
                                                    Lunas Terverifikasi
                                                </Badge>
                                            </TableCell>
                                            <TableCell class="text-center">
                                                <Button @click="openModal(pembayaran)" size="sm" variant="outline" class="bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100 hover:text-emerald-800">
                                                    <Eye class="w-4 h-4 mr-1.5" /> Lihat Arsip
                                                </Button>
                                            </TableCell>
                                        </TableRow>
                                    </template>
                                        ...
                                        <TableRow>
                                            <TableCell colspan="8" class="h-64 text-center">
                                                <div class="max-w-md mx-auto py-12">
                                                    ...
                                                    <p class="text-base font-semibold text-gray-900">Belum ada pembayaran yang disetujui</p>
                                                    <p class="text-sm text-gray-500 mt-1">Arsip dan riwayat pembayaran yang telah diverifikasi lunas akan muncul di sini.</p>
                                                </div>
                                            </TableCell>
                                        </TableRow>
                                    </template>
        ...
        <Dialog v-model:open="isModalOpen">
            <DialogContent class="max-w-2xl bg-white p-6 rounded-2xl shadow-2xl">
                <DialogHeader class="bg-[#4B49AC] text-white p-6 -m-6 mb-6 rounded-t-2xl">
                    <DialogTitle class="text-xl font-bold text-white">Arsip Bukti Pembayaran KRS</DialogTitle>
                    <DialogDescription class="text-indigo-100 text-sm mt-1">
                        Detail riwayat pembayaran mahasiswa yang telah terverifikasi lunas.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div v-if="selectedPembayaran" class="space-y-6">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                        <Button 
                            ...
                            class="bg-[#4B49AC] hover:bg-[#3f3d91] text-white font-semibold px-6"
                        >
                            Tutup Arsip
                        </Button>
                    </div>
