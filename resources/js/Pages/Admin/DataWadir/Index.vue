...
const updateFilters = () => {
  ...
  filterTimeout = setTimeout(() => {
    router.get('/v2/admin/data-master/data-wadir', {
      search: search.value,
    }, {
      preserveState: true,
      replace: true
...
const isDuplicateAdd = computed(() => {
  ...
  return props.wadirs.data.some(w => String(w.dosens_id) === String(form.dosens_id))
})
...
// Actions
const openEditModal = (wadir) => {
  selectedWadir.value = wadir
  editForm.id = wadir.id
  editForm.no = wadir.no
  editForm.status = wadir.status
  editForm.password = ''
...
const submitAdd = () => {
  ...
  form.post('/v2/admin/data-master/data-wadir', {
    onSuccess: () => {
...
const submitUpdate = () => {
  editForm.put(`/v2/admin/data-master/data-wadir/${editForm.id}`, {
    onSuccess: () => {
...
const confirmDelete = (wadir) => {
  selectedWadir.value = wadir
  isDeleteModalOpen.value = true
}

const submitDelete = () => {
  router.delete(`/v2/admin/data-master/data-wadir/${selectedWadir.value.id}`, {
    onSuccess: () => {
...
<template>
  <AdminLayout>
    <Head title="Kelola Data Wakil Direktur" />

    <div class="space-y-6">
      <!-- Header Section -->
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-[#1F2937]">Kelola Data Wakil Direktur</h1>
          <p class="text-[#6B7280]">Manajemen pejabat Wakil Direktur institusi.</p>
        </div>
        ...
        <CardContent class="p-4">
          <div class="flex flex-col md:flex-row gap-4 items-end">
            <div class="space-y-2 flex-1">
              <Label class="text-xs font-semibold text-[#374151]">Cari Wadir</Label>
              <div class="relative">
      ...
      <Card class="border-none shadow-sm overflow-hidden">
        <Table>
          <TableHeader class="bg-[#F9FAFB]">
            <TableRow>
              <TableHead class="font-bold text-[#374151]">Posisi</TableHead>
          ...
          <TableBody>
            <TableRow v-for="wadir in wadirs.data" :key="wadir.id" class="hover:bg-gray-50/50 transition-colors">
              <TableCell>
              ...
              <TableCell>
                <div>
                  <div class="font-semibold text-[#1F2937]">{{ wadir.nama }}</div>
                  <div class="text-xs text-[#6B7280]">{{ wadir.email }}</div>
                </div>
              </TableCell>
              <TableCell class="text-[#4B5563]">{{ wadir.no_telephone }}</TableCell>
              <TableCell>
                <span v-if="wadir.status == 1" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700">
                  Aktif
                </span>
              ...
              <TableCell class="text-right">
                <div class="flex justify-end gap-2">
                  <Button variant="ghost" size="sm" @click="openEditModal(wadir)" class="text-blue-600 hover:text-blue-700 hover:bg-blue-50 h-8 w-8 p-0">
                    <Pencil class="w-4 h-4" />
                  </Button>
                  <Button variant="ghost" size="sm" @click="confirmDelete(wadir)" class="text-red-600 hover:text-red-700 hover:bg-red-50 h-8 w-8 p-0">
                    <Trash2 class="w-4 h-4" />
                  </Button>
                </div>
              </TableCell>
            </TableRow>
            <TableRow v-if="wadirs.data.length === 0">
              <TableCell colspan="5" class="h-32 text-center text-[#9CA3AF]">
                <div class="flex flex-col items-center justify-center space-y-2">
                  <Shield class="w-8 h-8 opacity-20" />
                  <p>Tidak ada data Wakil Direktur yang ditemukan.</p>
                </div>
              </TableCell>
            </TableRow>
          </TableBody>
        ...
        <div class="p-4 border-t border-gray-100 flex items-center justify-between bg-white">
          <p class="text-sm text-[#6B7280]">
            Menampilkan {{ wadirs.from || 0 }} sampai {{ wadirs.to || 0 }} dari {{ wadirs.total }} data
          </p>
          <div class="flex items-center gap-2">
            <Button 
              variant="outline" 
              size="sm" 
              :disabled="!wadirs.prev_page_url"
              @click="router.get(wadirs.prev_page_url, {}, { preserveState: true })"
            ...
            <div class="flex items-center gap-1">
              <Button 
                v-for="p in wadirs.links.slice(1, -1)" 
                :key="p.label"
                variant="ghost"
                size="sm"
            ...
            <Button 
              variant="outline" 
              size="sm" 
              :disabled="!wadirs.next_page_url"
              @click="router.get(wadirs.next_page_url, {}, { preserveState: true })"
              class="rounded-lg h-8 w-8 p-0"
            >
    ...
    <Sheet :open="isAddModalOpen" @update:open="isAddModalOpen = $event">
      <SheetContent side="right" class="sm:max-w-[35%] w-full p-0 border-none shadow-2xl bg-white flex flex-col">
        <div class="bg-[#4B49AC] p-6 text-white shrink-0">
          <SheetHeader>
            <SheetTitle class="text-xl font-bold text-white">Tambah Wakil Direktur</SheetTitle>
            <SheetDescription class="text-indigo-100 mt-1">
              Pilih dosen yang akan ditugaskan sebagai Wadir.
            </SheetDescription>
          </SheetHeader>
        </div>
            ...
            <div class="space-y-4">
              ...
              <div v-if="isDuplicateAdd" class="p-3 bg-red-50 border border-red-100 rounded-lg flex items-start gap-3 animate-in fade-in zoom-in duration-300">
                ...
                <div class="text-xs text-red-700 leading-relaxed font-medium">
                  Dosen ini sudah terdaftar sebagai Wakil Direktur.
                </div>
              </div>
              ...
              <div class="space-y-2">
                <Label class="text-xs font-bold text-gray-500 uppercase tracking-wider">Posisi (Nomor)</Label>
                <Input v-model="form.no" type="number" placeholder="Contoh: 1 (Wadir I), 2 (Wadir II)..." class="h-11 rounded-lg border-gray-200" required />
                <p v-if="form.errors.no" class="text-xs text-red-500 font-medium">{{ form.errors.no }}</p>
              </div>
                ...
                <div class="grid grid-cols-1 gap-3">
                  ...
                  >
                    ...
                    <div class="flex-1">
                      ...
                      <p class="text-[10px] text-gray-500 italic">Gunakan password jabatan (Kaprodi/Lainnya) yang sudah ada.</p>
                    </div>
    ...
    <Sheet :open="isEditModalOpen" @update:open="isEditModalOpen = $event">
      <SheetContent side="right" class="sm:max-w-[35%] w-full p-0 border-none shadow-2xl bg-white flex flex-col">
        <div class="bg-[#4B49AC] p-6 text-white shrink-0">
          <SheetHeader>
            <SheetTitle class="text-xl font-bold text-white">Edit Wakil Direktur</SheetTitle>
            <SheetDescription class="text-indigo-100 mt-1">
              Perbarui status atau password Wadir.
            </SheetDescription>
          </SheetHeader>
        </div>
          ...
          <div class="flex-1 overflow-y-auto p-6 space-y-6">
            <div class="space-y-4">
              <div class="p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                ...
                <div class="text-sm font-bold text-gray-800">{{ selectedWadir?.nama }}</div>
                <div class="text-xs text-indigo-600">{{ selectedWadir?.email }}</div>
              </div>

              <div class="space-y-2 pt-4">
                <Label class="text-xs font-bold text-gray-500 uppercase tracking-wider">Posisi (Nomor)</Label>
                <Input v-model="editForm.no" type="number" class="h-11 rounded-lg border-gray-200" required />
                <p v-if="editForm.errors.no" class="text-xs text-red-500 font-medium">{{ editForm.errors.no }}</p>
              </div>
        ...
        <div class="p-8 text-center">
          ...
          <DialogHeader>
            ...
            <DialogDescription class="text-gray-500 text-center mt-3 px-2 text-base">
              Apakah Anda yakin ingin menghapus jabatan <span class="text-danger font-bold">Wadir {{ selectedWadir?.urutan }}</span> untuk <span class="text-danger font-bold">"{{ selectedWadir?.dosen?.nama }}"</span>? 
